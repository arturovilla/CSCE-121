...
Image::Image(string filename) : image(nullptr), width(0), height(0) {
    load(filename);
}

Image::Image(unsigned int width, unsigned int height)
: image(nullptr), width(width), height(height) {
...
Image::Image(const Image& p): width(p.width), height(p.height), filename(p.filename){
    if (width > 1 && height > 1) {
...
Image& Image::operator=(const Image& p){
    if(this != &p){
        clear();
        allocateImage(p.width, p.height);
